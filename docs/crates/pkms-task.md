...
`crates/pkms-task` owns task-domain behavior for local PKMS tasks. It does not
own CLI parsing or raw org text edits.
...
- Assign deterministic PKMS task IDs shared by `task list`, `task agenda`, and
  ID-first actions.
- Parse and apply task filters for state, tags, type, priority, dates, scope,
  and project.
- Model stable task items for structured output.
- Collect tasks from the local org provider.
...
- The source-neutral output shape remains stable for PKMS JSON/NDJSON
  compatibility even though remote providers are no longer implemented here.
