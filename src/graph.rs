...
mod tests {
    ...
    fn test_self_link_not_broken() {
        let results = vec![make_note("self", "Self Link", vec![Link::Internal("self".to_string())])];
        let graph = Graph::build(results);
        assert_eq!(graph.broken_links.len(), 0);
    ...
    fn test_url_and_file_links_not_counted_as_broken() {
        let results = vec![make_note(
            "a",
            "Note A",
            vec![Link::Url("https://example.com".to_string()), Link::File("/tmp/test".to_string())],
        )];
        let graph = Graph::build(results);
        assert_eq!(graph.broken_links.len(), 0);
    }
    ...
    fn test_notes_by_tag() {
        let results = vec![
            ...
            make_note_full("b", "Tagged B", vec![], vec!["foo".to_string(), "bar".to_string()], vec![]),
    ...
    fn test_all_tags() {
        let results = vec![
            ...
            make_note_full("b", "B", vec![], vec!["alpha".to_string(), "beta".to_string()], vec![]),
