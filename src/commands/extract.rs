...
impl TryFrom<&ExtractArgs> for ExtractOptions {
    type Error = anyhow::Error;

    fn try_from(args: &ExtractArgs) -> Result<Self> {
...
fn render(ctx: &OutputContext, output: &ExtractOutput) -> Result<()> {
    if ctx.is_structured() {
        ...
        if output.applied {
            ...
            println!("Would extract heading:");
        }
        println!("  Title:       {}", output.title);
        println!("  UUID:        {}", output.uuid);
        println!("  Source:      {}", output.source_path.display());
        println!("  New note:    {}", output.new_path.display());
        println!("  Replacement: {}", output.replacement);
        if output.applied {
            println!("  Status:      applied");
        } else {
            println!("  Status:      dry-run (use --apply to write)");
        }
    }
